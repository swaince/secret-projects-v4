...
defineOptions({ name: 'OperationLogIndex' })
...
import type { OperationLogDTO, OperationLogPageParams } from '@/api/log'
import { fetchOperationLogs, fetchOperationLogDetail } from '@/api/log'
...
const searchControllerMethod = ref('')
...
const detailLog = ref<OperationLogDTO | null>(null)
...
async function loadLogs() {
  const params: OperationLogPageParams = { page: page.value, size: size.value }
  ...
  if (searchControllerMethod.value) params.controllerMethod = searchControllerMethod.value
  if (searchStartTime.value) params.startTime = searchStartTime.value
  if (searchEndTime.value) params.endTime = searchEndTime.value
  const res: PageResponse<OperationLogDTO> = await fetchOperationLogs(params)
...
async function openDetail(log: OperationLogDTO) {
  detailLog.value = null
  detailLoading.value = true
  dialogOpen.value = true
  try {
    detailLog.value = await fetchOperationLogDetail(log.logId)
  } finally {
    ...
    <Card class="py-4">
      <CardContent>
        <div class="grid grid-cols-4 gap-4">
          ...
          <div class="flex items-center gap-2">
            <Label class="shrink-0">方法</Label>
            <Input
              v-model="searchControllerMethod"
              name="controllerMethod"
              autocomplete="off"
              placeholder="controller方法…"
              class="flex-1"
            />
          </div>
          <div class="flex items-center gap-2">
        ...
        <Table class="table-fixed">
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Controller方法</TableHead>
          ...
          <TableBody>
            ...
            >
              ...
              <TableCell class="truncate">{{ log.controllerMethod ?? '-' }}</TableCell>
            ...
            <TableEmpty v-if="logs.length === 0" :colspan="8">暂无数据</TableEmpty>
          </TableBody>
        </Table>
      ...
      <DialogContent class="sm:max-w-2xl">
        ...
        <div v-else-if="detailLog" class="max-h-[70vh] overflow-y-auto space-y-5 py-2">
          ...
          <div class="flex items-center gap-2 text-sm">
            <div class="min-w-0">
              <span class="text-muted-foreground text-xs">处理器</span>
              <p class="break-all font-mono">{{ detailLog.controllerClass && detailLog.controllerMethod ? detailLog.controllerClass + '#' + detailLog.controllerMethod : '-' }}</p>
            </div>
            <Button v-if="detailLog.controllerClass" variant="ghost" size="sm" class="size-7 shrink-0" aria-label="复制处理器" @click="copyToClipboard(detailLog.controllerClass + '#' + detailLog.controllerMethod, 'handler')">
              <CopyCheck v-if="copiedId === 'handler'" class="size-3.5 text-green-600" />
              <Copy v-else class="size-3.5" />
            </Button>
          </div>
